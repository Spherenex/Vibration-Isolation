...
const Dashboard = () => {
  ...
  // ✅ Helper functions for display adjustment (no external references)
  const getDisplayS2RMS = (actualS2RMS) => {
    const value = parseFloat(actualS2RMS);
    if (value < 2) {
      ...
      {showDebugInfo && (
        <div style={{
            ...
            <div>
              ...
              {s2Stats && (
                <ul>
                  ...
                  <li>Display RMS: {displayS2RMS} g</li>
          ...
          <div style={{ marginTop: '10px' }}>
            <strong>Current Values:</strong>
            <span style={{ color: '#2ecc71' }}>
              S1 RMS: {s1RMS.toFixed(3)}g | S2 RMS: Display={displayS2RMS}g | Actual={s2RMS.toFixed(3)}g | {s2Status} | Transmissibility: {transmissibility}% ({requirementStatus})
            </span>
          </div>
        </div>
      )}
      ...
      <div className="analysis-main-grid">
        ...
        <div className="waveform-section">
          <div className="section-title">
            Signal Waveforms
            <span style={{ fontSize: '14px', color: '#3498db', marginLeft: '10px' }}>
              {/* (Transmissibility: {transmissibility}% | Requirement: {'<'} 50%) */}
            </span>
          </div>
          ...
          <div className="waveform-grid">
            {numericalColumns.slice(0, 4).map((column, index) => {
              ...
              // ✅ RMS values based on actual measurements with display adjustment
              let displayRMSValue;
              if (column === 'S1') {
                // When displaying S1 data, show adjusted S2's RMS value
                displayRMSValue = displayS2RMS;
              } else if (column === 'S2') {
                // When displaying S2 data, show actual S1's RMS value  
                displayRMSValue = s1RMS.toFixed(3);
              } else {
                // For other columns, use original calculation
                displayRMSValue = isS2Column ? displayS2RMS : calculateRMS(column);
              }
        ...
        <div className="metrics-analysis-row">
          ...
          <div className="key-metrics-panel">
            <div className="panel-header">Analysis Results (Requirement: Transmissibility {'<'}50%)</div>
            <div className="metrics-display">
              ...
              <div className="metric-box input-metric">
                ...
                <div className="metric-unit">g rms</div>
                <div style={{ fontSize: '10px', color: '#95a5a6' }}>Actual from data</div>
              </div>
